  ...
  <div>
      <card :dis-hover="true">
        ...
        <div style="margin-bottom: 10px;">
          <Input clearable placeholder="文章标题" v-model="query.title" style="width: 250px;" />
        ...
        <Table :loading="table.loading" :columns="table.columns" :data="table.data" stripe border ref="selection" @on-delete="handleDelete" @on-select="handleSelect">
          <template slot="action" slot-scope="{ row, index }">
            <Button @click="toEditModel(index)" type="primary" size="small">编辑</Button>
            <Button @click="handleDelete(index)" type="warning" size="small" style="margin-left: 5px">删除</Button>
            <Button @click="handleFlushDelete(index)" type="error" size="small" style="margin-left: 5px">彻底删除</Button>
          </template>
        </Table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toEditModel(index) {
        this.$emit('editArticle',this.table.data[index].id);
      },
